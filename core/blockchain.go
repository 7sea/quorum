...
type BlockChain struct {
	...
	extensions chan ChainHeadEvent // Serialized chain extension events
}
...
func NewBlockChain(chainDb ethdb.Database, config *ChainConfig, pow pow.PoW, mux *event.TypeMux, enableQuorumChecks bool) (*BlockChain, error) {
	...
	bc := &BlockChain{
		...
		pow:          pow,
		extensions:   make(chan ChainHeadEvent),
	}
...
func (self *BlockChain) InsertChain(chain types.Blocks) (int, error) {
	...
	// ChainHeadEvents. This is important for supporting high-throughput/
...
// When we update the head of the blockchain we must publish a
// `ChainHeadEvent`. `eventMux.Post` cannot be called while we are holding
// `mu`, and we need to maintain a correct ordering of these events for
// correctness, so they are published into the `extensions` channel, and
// a goroutine running this loop performs the `Post`.
func (self *BlockChain) chainExtensionLoop() {
	for chainHeadEvent := range self.extensions {
		self.eventMux.Post(chainHeadEvent)
	}
}
...
// high-throughput, non-PoW settings.
	...
	for _, event := range events {
		if event, ok := event.(ChainEvent); ok {
			if currBlockHash == event.Hash {
				// Serialize all CHEs through the `chainExtensionLoop` worker:
				self.extensions <- ChainHeadEvent{event.Block}
			}
		}
		// Fire the insertion events individually too
		go self.eventMux.Post(event)
	}
}
